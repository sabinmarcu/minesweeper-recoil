...
const matchMediaSetToString = (set: MatchMediaSet): string =>
  typeof set === "string" ? set : set.join(": ");

const matchMediaSetListToString = (sets: MatchMediaSet[]): string =>
  sets
...
): AtomEffect<T> => ({ trigger, setSelf }) => {
  let mediaString = matchMediaSetListToString(media);
